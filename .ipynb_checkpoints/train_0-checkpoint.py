import os
import argparse
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,3'  #'3,2' #'3,2,1,0'
DEBUG=0
from common import *
...
parser = argparse.ArgumentParser(
    description='Train Mask R-CNN on DSB2018.')
parser.add_argument('--split', required=False,
                    default=split,
                    metavar="split set name",
                    help='The set of name for certain split')
parser.add_argument('--out-dir', required=False,
                    default=out_dir,
                    metavar="path/to/save_directory",
                    help='Directory for save the output')
parser.add_argument('--checkpoint', required=False,
                    default=initial_checkpoint,
                    metavar="path/to/checkpoint",
                    help='Directory to the checkpoint to be loaded')
...
def train_augment(image, multi_mask, meta, index):
    ...
    input = torch.from_numpy(image.transpose((2,0,1))).float().div(255)
    box, label, instance  = multi_mask_to_annotation(multi_mask)
    foreground = (multi_mask!=0).astype(np.float32)
...
def train_collate(batch):
    ...
    return [inputs, boxes, labels, instances, metas, indices, foregrounds]
...
def evaluate( net, test_loader ):
    ...
    test_acc  = 0
    for i, (inputs, truth_boxes, truth_labels, truth_instances, metas, indices, truth_foregrounds) in enumerate(test_loader, 0):
        ...
        test_loss += batch_size*np.array((
                       ...
                       net.unet_loss.cpu().data.numpy(),
                       net.crf_loss.cpu().data.numpy()
                     ))
        test_num  += batch_size
...
def run_train():
    ...
    # out_dir  = RESULTS_DIR + '/mask-rcnn-0315'
    # initial_checkpoint = None
    #RESULTS_DIR + '/mask-rcnn-0314/checkpoint/00005500_model.pth'
        ##
    ...
    print("Split: ", split)
    print("initial_checkpoint: ", initial_checkpoint)
    ...
    if batch_size == 1:
        ...
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),
                              lr=2e-4 / iter_accum, momentum=0.9, weight_decay=1e-4)
    else:
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),
                          lr=0.01/iter_accum, momentum=0.9, weight_decay=1e-4)
    ...
    log.write('\ttrain_dataset.set_name = %s\n'%(train_dataset.set_name))
    log.write('\tvalid_dataset.set_name = %s\n'%(valid_dataset.set_name))
    ...
    log.write(' rate    iter   epoch  num   | valid_loss               | train_loss               | time          \n')
    log.write('-------------------------------------------------------------------------------------------------------------------------------\n')
    ...
    while  i<num_iters:  # loop over the dataset multiple times
        ...
        net.set_mode('train')
        optimizer.zero_grad()
        for inputs, truth_boxes, truth_labels, truth_instances, metas, indices, truth_foregrounds in train_loader:
            ...
            if i % iter_valid==0:
                ...
                log.write('%0.4f %5.1f k %6.1f %4.1f m | %0.3f   %0.2f %0.2f   %0.2f %0.2f   %0.2f | %0.3f   %0.2f %0.2f   %0.2f %0.2f   %0.2f | %s\n' % (\
            ...
            print('\r%0.4f %5.1f k %6.1f %4.1f m | %0.3f   %0.2f %0.2f   %0.2f %0.2f   %0.2f | %0.3f   %0.2f %0.2f   %0.2f %0.2f   %0.2f | %s  %d,%d,%s' % (\
                         rate, i/1000, epoch, num_products/1000000,
                         valid_loss[0], valid_loss[1], valid_loss[2], valid_loss[3], valid_loss[4], valid_loss[5],#valid_acc,
                         train_loss[0], train_loss[1], train_loss[2], train_loss[3], train_loss[4], train_loss[5],#train_acc,
